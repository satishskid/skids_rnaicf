...
// Silent upgrades are a safety regression for clinical use.
//
// All shards are served same-origin from the SKIDS worker, which proxies the
// R2 bucket `skids-models`. Browsers never contact huggingface.co, mlc.ai,
// or any third-party CDN during model load.
//
// When the Liquid AI model is re-released, update the version, shard list,
// and per-shard sha256 values together in a single reviewable diff.
