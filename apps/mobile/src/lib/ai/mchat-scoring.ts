 ...
 *
 * Validated ASD risk screening for children 16–30 months.
 * 20 yes/no items: Low (0–2) / Medium (3–7) / High (8–20 risk).
 *
...
/** Score M-CHAT-R responses per published algorithm. */
export function scoreMChat(answers: MChatAnswer[]): MChatResult {
  ...
  for (const item of MCHAT_ITEMS) {
    ...
    if (!passed) {
      ...
      if (item.critical) criticalFailedItems.push(item.id)
    }
  }
  ...
  return { totalScore, criticalScore, risk, failedItems, criticalFailedItems, domainScores, recommendation }
}

/** Map M-CHAT results to observation features for storage. */
export function mchatToFeatures(result: MChatResult): Record<string, unknown> {
  return {
    ...
    riskCategory: result.risk === 'high' ? 'high_risk' : result.risk === 'medium' ? 'possible_risk' : 'no_risk',
  }
}
