...
const MODULE_TYPE = 'posture'
const CAMERA_FACING = 'environment' as const

export function PostureScreening({
  ...
  const capturePhoto = useCallback(() => {
    ...
    // No AI color analysis for posture — structural assessment only
    setStep(2)
  }, [setStep])
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="relative aspect-[3/4] bg-gray-900 rounded-lg overflow-hidden">
            ...
            <MediaPipeOverlay videoRef={videoRef as React.RefObject<HTMLVideoElement>} tasks={['pose']} />
            <div className="absolute bottom-4 left-0 right-0 text-center text-white text-sm bg-black/30 py-1">
              {childName} standing straight — back to camera
            </div>
          </div>
