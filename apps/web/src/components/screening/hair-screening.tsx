...
const MODULE_TYPE = 'hair'
const CAMERA_FACING = 'environment' as const

export function HairScreening({
  ...
  const handleComplete = () => {
    ...
    onComplete({ moduleType: MODULE_TYPE, annotationData, lfmResult: lfmResult || undefined, riskCategory: selectedChips.length === 0 ? 'no_risk' : 'possible_risk' })
  }
        ...
        <CardContent className="space-y-4">
          <div className="relative aspect-[4/3] bg-gray-900 rounded-lg overflow-hidden">
            ...
            <div className="absolute bottom-4 left-0 right-0 text-center text-white text-sm bg-black/30 py-1">
              Part hair and photograph scalp sections
            </div>
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {capturedImage && (
        <Card>
          <CardHeader className="pb-2"><CardTitle>Outline Patches</CardTitle></CardHeader>
          <CardContent>
            <TapToSegment imageBase64={capturedImage} onSegmentComplete={() => {}} />
          </CardContent>
        </Card>
      )}
      <Card>
