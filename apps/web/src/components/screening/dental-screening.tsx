...
const MODULE_TYPE = 'dental'

export function DentalScreening({
  ...
  // MobileSAM tap-to-segment handler
  const handleSegmentTap = async (x: number, y: number) => {
    if (!bestFrame || samRunning) return
    setSamRunning(true)
    try {
      const { segmentWithPoints } = await import('@/lib/ai/segmentation')
      // Load image to get natural dimensions for coordinate mapping
      const img = new Image()
      await new Promise<void>((resolve) => { img.onload = () => resolve(); img.src = bestFrame })
      // Convert percentage coords to pixel coords in display space
      const displayW = img.naturalWidth
      const displayH = img.naturalHeight
      const px = Math.round(x * displayW)
      const py = Math.round(y * displayH)
      ...
      if (result) {
        setSamResult(result)
        setSamOverlay(result.maskDataUrl)
      }
    } catch { /* SAM not available — silent */ }
    ...
    return (
      <div className="space-y-4">
        ...
        <VideoCapture
          ...
          instructions={`Ask ${childName} to open mouth wide. Record all quadrants — upper, lower, left, right. Use phone flashlight.`}
        />
      </div>
    )
  }
    ...
    <div className="space-y-4">
        ...
        <CardContent>
          ...
          {samOverlay && (
            <div className="relative mt-1">
              <img src={samOverlay} alt="SAM segmentation" className="absolute inset-0 w-full h-full object-cover pointer-events-none rounded-lg" style={{ mixBlendMode: 'multiply' }} />
            </div>
          )}
          {/* SAM controls */}
          <div className="flex items-center gap-2 mt-2">
            <Button variant="outline" size="sm" className="text-xs" onClick={() => {
              if (bestFrame) {
                // Trigger a segment at image center as demo
                handleSegmentTap(0.5, 0.5)
              }
            }} disabled={samRunning || !bestFrame}>
              {samRunning ? (
                <><Icons.Loader2 className="w-3 h-3 animate-spin mr-1" /> Segmenting...</>
              ) : (
                <><Icons.Scan className="w-3 h-3 mr-1" /> Tap to Segment</>
              )}
            </Button>
            {samResult && (
              <span className="text-[10px] text-gray-500">Area: {samResult.area}% · {samResult.inferenceTimeMs}ms</span>
            )}
            {samOverlay && (
              <Button variant="ghost" size="sm" className="text-xs text-gray-400" onClick={() => { setSamOverlay(null); setSamResult(null) }}>
                Clear
              </Button>
            )}
          </div>
      ...
      <Card><CardContent className="pt-4">
        <AnnotationChips chips={chips} selectedChips={selectedChips}
          ...
          moduleType="dental" moduleName="Dental Screening"
