...
const MODULE_TYPE = 'abdomen'
const CAMERA_FACING = 'environment' as const

export function AbdomenScreening({
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="p-3 bg-blue-50 rounded-lg text-xs text-blue-700">
            <strong>Tip:</strong> Use location pins to mark tenderness quadrants (RUQ, LUQ, RLQ, LLQ) on the photo after capture.
          </div>
        ...
        <CardContent className="space-y-4">
          <div className="relative aspect-[4/3] bg-gray-900 rounded-lg overflow-hidden">
            ...
            <div className="absolute bottom-4 left-0 right-0 text-center text-white text-sm bg-black/30 py-1">
              {childName} lying supine — photograph abdomen
            </div>
          </div>
    ...
    <div className="space-y-4">
      <Card><CardHeader className="pb-2"><CardTitle>Annotate — {instructions.title}</CardTitle></CardHeader>
        <CardContent>
          ...
          {capturedImage && (
            <div className="mt-3">
              <TapToSegment imageBase64={capturedImage} />
            </div>
          )}
        </CardContent></Card>
      {capturedImage && (
...
export default AbdomenScreening
